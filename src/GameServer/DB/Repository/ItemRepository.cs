...
public sealed class ItemRepository
{
    ...
    public async Task<long> AcquireItemAsync(long playerId, int itemDefId, int count)
    {
        ...
        {
            cmd.Transaction = tx;
            cmd.CommandText = "INSERT INTO item_instance (item_def_id) VALUES (@defId)";
            cmd.Parameters.AddWithValue("@defId", itemDefId);
            await cmd.ExecuteNonQueryAsync();
            itemInstanceId = cmd.LastInsertedId;
        }
        ...
        await using (var cmd = conn.CreateCommand())
        {
            cmd.Transaction = tx;
            cmd.CommandText =
                "INSERT INTO inventory (player_id, item_instance_id, item_count) VALUES (@pid, @iid, @count)";
            cmd.Parameters.AddWithValue("@pid", playerId);
            cmd.Parameters.AddWithValue("@iid", itemInstanceId);
            cmd.Parameters.AddWithValue("@count", count);
            await cmd.ExecuteNonQueryAsync();
            inventoryId = cmd.LastInsertedId;
        }
